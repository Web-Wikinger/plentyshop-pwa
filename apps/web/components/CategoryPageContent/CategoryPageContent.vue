<template>
  <NarrowContainer class="mb-20 px-4 md:px-0" data-testid="category-layout">
    <div class="md:flex gap-6 pt-8" data-testid="category-page-content">
      ...
      <div class="flex-1">
        ...
        <section v-if="products?.length"
          ...
          <NuxtLazyHydrate when-visible v-for="(product, index) in products" :key="index">
            <UiProductCard :product="product" :name="productGetters.getName(product) ?? ''"
                           :rating-count="productGetters.getTotalReviews(product)"
                           :rating="productGetters.getAverageRating(product, 'half')"
                           :image-url="addModernImageExtension(productGetters.getCoverImage(product))" :image-alt="productImageGetters.getImageAlternate(productImageGetters.getFirstImage(product)) ||
        ...
        <UiPagination v-if="totalProducts > 0" :key="`${totalProducts}-${itemsPerPage}`"
                      :current-page="getFacetsFromURL().page ?? 1" :total-items="totalProducts"
...
import { productGetters, productImageGetters, cartGetters, CartItem, Cart, Product } from '@plentymarkets/shop-api';

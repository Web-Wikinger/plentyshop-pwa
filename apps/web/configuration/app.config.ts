export const appConfiguration = {
  head: {
    ...
    meta: [
      { name: 'shop-name', content: process.env.STORENAME || 'Kellogg\'s Shop' },
      { name: 'description', content: process.env.METADESC || 'Welcome to Kellogg\'s Shop' },
      { name: 'keywords', content: process.env.METAKEYWORDS || 'Kelloggs, Kelloggs Shop' },
      { name: 'theme-color', content: '#0C7992' },
      { property: 'og:title', content: process.env.OGTITLE || 'Kellogg\'s Shop' },
    ...
    title: process.env.STORENAME || 'Kellogg\'s Shop',
  },
};
